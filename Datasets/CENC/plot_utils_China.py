...
def plot_borders(borders,ax):
	for line in borders:
		...
		if len(line) > 1 and len(line) % 2 == 0:
			ax.plot(line[0::2], line[1::2], '-', color='#A9A9A9', transform=ccrs.PlateCarree(),zorder=100,linewidth=0.7) #zorder 控制画图顺序,越大画上去越晚
		else:
...
def points(lats, lons):
    pg = Polygon(np.load('/home/tianweixi/Programe/EarthquakeNPP_CSEP_China/Datasets/CENC/data/CSEP_COLLECTION_REGION.npy'))
