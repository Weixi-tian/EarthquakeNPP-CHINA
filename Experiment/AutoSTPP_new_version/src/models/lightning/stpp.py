...
from utils import scale_ll, scale_ll_all_events
...
class BaseSTPointProcess(pl.LightningModule):
    ...
    def calc_norm(self, st_x, st_x_cum): #归一化
        ...
        self.scales = (st_x_cum[0, 2, :] - st_x_cum[0, 1, :]) / (st_x[0, 2, :] - st_x[0, 1, :]) #why?
        self.scales = self.scales.numpy()
        self.biases = st_x_cum[0, 2, :] - st_x[0, 2, :] * self.scales
        self.biases = self.biases.numpy()

        #added by tian weixi  删除 biases 中的 magnitude 部分（第 3 列） 
        #========================================================================
        if st_x.shape[-1] == 4:
            print('田唯熙检查scales的形状和值：',self.scales) 
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.model_type.endswith("_deepstpp_mag") or self.model_type.endswith("_deepstpp"): 
            nll, sll, tll,sll_all,tll_all = self(st_x, st_y) # self(st_x,st_y) 表示调用该类的forward 方法 （实际上是使用子类DeepSTPP的forward 方法得到结果）
            if type(self.scales) is list:
                self.calc_norm(st_x, st_x_cum)
            nll_scaled, sll_scaled, tll_scaled,sll_all_scaled,tll_all_scaled = scale_ll_all_events(None, nll, sll, tll,sll_all,tll_all, self.scales) #归一化
        else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #added by tianweixi (if ... else...)
        if self.model_type.endswith("_deepstpp_mag") or self.model_type.endswith("_deepstpp"):
            nll, sll, tll,sll_all,tll_all  = self(st_x, st_y)
            nll_scaled, sll_scaled, tll_scaled,sll_all_scaled,tll_all_scaled = scale_ll_all_events(None, nll, sll, tll,sll_all,tll_all, self.scales) #归一化
        else:
            nll, sll, tll = self(st_x, st_y) 
            nll_scaled, sll_scaled, tll_scaled = scale_ll(None, nll, sll, tll, self.scales)
    ...
    def test_step(self, batch, batch_idx):
        ...
        #added by tianweixi (if ... else...)
        if self.model_type.endswith("_deepstpp_mag") or self.model_type.endswith("_deepstpp"): #added by tianweixi (if ... else...)
            print('deepstpp and deepstppmag model will generate LL values for each earthquake')
            nll, sll, tll,sll_all,tll_all = self(st_x, st_y)
            if type(self.scales) is list:
                self.calc_norm(st_x, st_x_cum)
            nll_scaled, sll_scaled, tll_scaled,sll_all_scaled,tll_all_scaled = scale_ll_all_events(None, nll, sll, tll,sll_all,tll_all, self.scales) #归一化
            ...
            self.results.append({
                ...
                "sll_mean": sll.item(),
                "tll_mean": tll.item(),
                "sll_mean_scaled": sll_scaled.item(), #最终的结果是用每个batch计算出来的放缩后的sll和tll求均值得到的
                "tll_mean_scaled": tll_scaled.item(),
                "sll_all": sll_all.tolist(),
                "tll_all": tll_all.tolist(),
                "sll_all_scaled": sll_all_scaled.tolist(),
                "tll_all_scaled": tll_all_scaled.tolist(),
            })
