...
def get_t0_t1(data):
    if data == "citibike":
        ...
        return torch.tensor([0.0]), torch.tensor([7.332638888888889])
    elif data == "CENC_20":
        return torch.tensor([0.0]), torch.tensor([7.332638888888889])
    elif data == "CENC_25":
        return torch.tensor([0.0]), torch.tensor([8.440277777777778])
    elif data == "CENC_30":
        return torch.tensor([0.0]), torch.tensor([23.986805555555556])
    elif data == "CENC_35":
        return torch.tensor([0.0]), torch.tensor([53.05637731481482])
    else:
...
def _main(rank, world_size, args, savepath, logger):
    ...
    print('train_epoch_iter length:',len(train_epoch_iter)) #try to under stand the length of the 'train_epoch_iter'
