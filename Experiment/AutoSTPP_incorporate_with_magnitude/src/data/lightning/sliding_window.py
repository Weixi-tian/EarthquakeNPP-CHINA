...
class SlidingWindowDataModule(pl.LightningDataModule):
    ...
    def __init__(
        self, 
        name: str = 'sthp0', 
        data_dir: str = "./data/spatiotemporal/", #存储数据的路径
        option: str = "ready", 
        batch_size: int = 128,
        num_workers: int = 8,
        magnitude: int = 1 # added by weixi tian 
    ):
        ...
        if self.hparams.option == 'ready': #判断是下载数据还是直接使用已经存在的数据
            pass
    ...
    def val_dataloader(self): #加载验证数据集
        return DataLoader(
            self.sliding_val, 
            batch_size=self.hparams.batch_size, 
            num_workers=self.hparams.num_workers,
    ...
    def test_dataloader(self): #加载测试数据集
        return DataLoader(
            self.sliding_test, 
            batch_size=self.hparams.batch_size, 
            num_workers=self.hparams.num_workers,
