...
from utils import scale_ll, scale_ll_all_events,scale_ll_all_events_mag1,scale_ll_all_events_mag2
...
class BaseSTPointProcess(pl.LightningModule):
    ...
    def __init__(
        ...
        self.scales_mag = [1., 1., 1.,1] #带有震级时的归一化的比例因子
        self.biases_mag = [0., 0., 0.,0]

        ##added by tianweixi
        #========================================================================
        self.results = []  # 初始化存储测试结果的列表 
        self.parameters_results = [] # 初始化存储参数的列表 
        self.name = name+'_'+'seed'+'_'+str(seed) #for automatic capture the name
        self.model_type = name # for decide whether calculate tll_all and s_ll_all
    ...
    def calc_norm(self, st_x, st_x_cum): #归一化
        ...
        self.biases = st_x_cum[0, 2, :] - st_x[0, 1, :] * self.scales
        self.biases = self.biases.numpy()

        #added by tian weixi  
        #========================================================================
        if st_x.shape[-1] == 4 and self.model_type.endswith("_deepstpp_mag2"):
            self.scales_mag = (st_x_cum[0, 2, :] - st_x_cum[0, 1, :]) / (st_x[0, 2, :] - st_x[0, 1, :])
            self.scales_mag = self.scales_mag.numpy()
            self.biases_mag = st_x_cum[0, 2, :] - st_x[0, 1, :] * self.scales_mag
            self.biases_mag = self.biases_mag.numpy()
            print('数据包括震级，scales的形状和值：',self.scales_mag)
        elif st_x.shape[-1] == 4 and self.model_type.endswith("_deepstpp_mag"):
            print('数据包括震级，scales的形状和值：',self.scales) 
            self.scales = np.delete(self.scales, 2)
            self.biases = np.delete(self.biases, 2)
        elif st_x.shape[-1] == 3:
            print('数据没有震级，检查scales的形状和值：',self.scales) 
        #========================================================================

    def training_step(self, batch, batch_idx):
        st_x, st_y, st_x_cum, _, _ = batch

        #added by tianweixi (if ... else...)

        if self.model_type.endswith("_deepstpp"):
            ...
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events(None, nll, sll, tll, sll_all, tll_all, self.scales)
        
        elif self.model_type.endswith("_deepstpp_mag"):
            nll, sll, tll, sll_all, tll_all,w_i,b_i,inv_var,s_diff,his_location = self(st_x, st_y)
            if type(self.scales) is list:
                self.calc_norm(st_x, st_x_cum)
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events_mag1(None, nll, sll, tll, sll_all, tll_all, self.scales)

        elif self.model_type.endswith("_deepstpp_mag2"): 
            nll, sll, tll, sll_all, tll_all,w_i,b_i,q_i,inv_var,s_diff,m_i,his_location = self(st_x, st_y)
            if type(self.scales_mag) is list:
                self.calc_norm(st_x, st_x_cum)
            # nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events(None, nll, sll, tll, sll_all, tll_all, self.scales)
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events_mag2(None, nll, sll, tll, sll_all, tll_all, self.scales_mag)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.model_type.endswith("_deepstpp") :
            nll, sll, tll, sll_all, tll_all,w_i,b_i,inv_var,s_diff,his_location = self(st_x, st_y)
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events(self.val_dataloader_ref, nll, sll, tll, sll_all, tll_all, self.scales)
        elif self.model_type.endswith("_deepstpp_mag"):
            nll, sll, tll, sll_all, tll_all,w_i,b_i,inv_var,s_diff,his_location = self(st_x, st_y)
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events_mag1(self.val_dataloader_ref, nll, sll, tll, sll_all, tll_all, self.scales)
        elif self.model_type.endswith("_deepstpp_mag2"): 
            nll, sll, tll, sll_all, tll_all,w_i,b_i,q_i,inv_var,s_diff,m_i,his_location  = self(st_x, st_y)
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events_mag2(self.val_dataloader_ref, nll, sll, tll, sll_all, tll_all, self.scales_mag)
        else:
            nll, sll, tll = self(st_x, st_y)
            nll_scaled, sll_scaled, tll_scaled = scale_ll(None, nll, sll, tll, self.scales)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.model_type.endswith("_deepstpp"):
            nll, sll, tll, sll_all, tll_all,w_i,b_i,inv_var,s_diff,his_location= self(st_x, st_y)
            if not isinstance(self.scales, (np.ndarray,list)):
                self.calc_norm(st_x, st_x_cum)
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events(self.test_dataloader_ref, nll, sll, tll, sll_all, tll_all, self.scales)
            ...
            if not isinstance(self.scales, (np.ndarray,list)):
                self.calc_norm(st_x, st_x_cum)
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events_mag1(self.test_dataloader_ref, nll, sll, tll, sll_all, tll_all, self.scales)
            ...
            if type(self.scales_mag) is list:
                self.calc_norm(st_x, st_x_cum)
            nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events_mag2(self.test_dataloader_ref, nll, sll, tll, sll_all, tll_all, self.scales_mag)
            # nll_scaled, sll_scaled, tll_scaled, sll_all_scaled, tll_all_scaled = scale_ll_all_events(None, nll, sll, tll, sll_all, tll_all, self.scales)
    ...
    # 在测试结束后保存所有结果 added by tianweixi
    #============================================================
    def save_results(self, file_path = None):
            """保存所有测试结果到文件"""
            if file_path is None:
                file_path = f"data/loglikelihood/LLscores_{self.name}.json"  # 使用时间戳避免覆盖
            # 确保路径存在
    ...
    def save_parameter_results(self, file_path = None):
            """保存所有测试结果到文件"""
            if file_path is None:
                file_path = f"data/parameters/parameter_{self.name}.json"  # 使用时间戳避免覆盖
            # 确保路径存在
            dir_path = os.path.dirname(file_path)  # 获取路径的目录部分
            if not os.path.exists(dir_path):  # 检查路径是否存在
                os.makedirs(dir_path)  # 创建路径
    ...
    def on_test_epoch_end(self): #对每个序列计算1.模型的时空强度 2.与真实强度的误差（MAE和Hessian） 3.可视化输出
        ...
        #============================================================
        if self.model_type.endswith("_deepstpp_mag") or self.model_type.endswith("_deepstpp_mag2") or self.model_type.endswith("_deepstpp"): #added by tianweixi (if ... else...)
