...
class TransformerEncoder(pl.LightningModule):    
    ...
    def __init__(
        ...
        **kwargs
        ...
        #Added by tian weixi for magnitude, the shape of input here shoud be [batch_size,seq_len,4]
        self.encoder = nn.Linear(4, self.hparams.emb_dim, bias=False) 
    ...
    def encode(self, x, x_mask=None): #x的形状一般为（batch_size, seq_len, features）
        ...
        x = self.encoder(x) * math.sqrt(self.ninp) #将输入的x从4维映射到128维，并对输入值进行缩放，目的是稳定数值
...
##=====================================================================
##=====================================================================
####change by tianweixi for DeepSTPP mag2 model (add m_i,m_c ,q_i here)

def ll_no_events(w_i, b_i, q_i, tn_ti, t_ti,m_i): #计算没有事件的似然函数，假设 强度函数λ(t)=w​⋅e−b​t
    """
    ...
    :param q_i: [batch, seq_len] add by tianweixi for DeepSTPP mag2 model

    :return: scalar 
    """
    #最终对每个seq_len求和，得到每个batch数据的总的似然函数[batch_size]
    return torch.sum(w_i / b_i * torch.exp(q_i*(m_i)) * (torch.exp(-b_i * t_ti) - torch.exp(-b_i * tn_ti)), -1) 


def log_ft(t_ti, tn_ti,m_i, w_i, b_i,q_i):
    return ll_no_events(w_i, b_i, q_i, tn_ti, t_ti,m_i) + torch.log(t_intensity(w_i, b_i, q_i, t_ti, m_i))


def t_intensity(w_i, b_i, q_i, t_ti, m_i):
    """
    ...
    :return: λ(t) [batch] <br>
             ...
             λ(s,t) [batch]
    """
    v_i = w_i * torch.exp(-b_i * t_ti) * torch.exp(q_i *(m_i)) #计算每个目标事件t_i的强度函数值
    lamb_t = torch.sum(v_i, -1) #（对所有目标事件的强度函数求和）
    return lamb_t


def s_intensity(w_i, b_i, q_i, t_ti, s_diff, inv_var, m_i):
    v_i = w_i * torch.exp(-b_i * t_ti) * torch.exp(q_i *(m_i))
    v_i = v_i / torch.sum(v_i, -1).unsqueeze(-1)  # Normalize    [batch,seq_len]
    g2 = torch.sum(s_diff * inv_var * s_diff, -1)
    g2 = torch.sqrt(torch.prod(inv_var, -1)) * torch.exp(-0.5 * g2) / (2 * np.pi) # [batch,seq_len]。这里的g2是根据高斯分布计算的每个历史事件对目标事件的影响，也就是目标事件S（x,y）在该历史事件的情况下的概率。
    f_s_cond_t = torch.sum(g2 * v_i, -1) #计算所有历史事件位置对目标事件S的影响，并结合归一化的时间概率分配比重，最终得到条件空间概率f(s|t),得到在所有历史事件的时空位置情况下，发生在目标空间位置的概率。
    return f_s_cond_t #[batch]

def intensity(w_i, b_i, q_i, t_ti, s_diff, inv_var, m_i): #定义条件强度函数 λ(s,t)=λ(t)⋅f(s∣t)
    return t_intensity(w_i, b_i, q_i, t_ti, m_i) * s_intensity(w_i, b_i, q_i, t_ti, s_diff, inv_var, m_i)

####change by tianweixi for DeepSTPP mag2 model (add m_i,m_c ,q_i above)
##=====================================================================
##=====================================================================
...
class DeepSTPointProcess(BaseSTPointProcess):
    ...
    def __init__(
        ...
        b_dec: Dict,
        s_dec: Dict,
        q_dec: Dict, # add by tianweixi for DeepSTPP mag2 model(for magnitude parameter alpha: )
        seq_len: int = 20,
        s_min: float = 1e-4,
        s_max: float = None,
        b_max: float = 20,
        ...
        **kwargs  # for BaseSTPointProcess
        ...
        q_dec['init_args'].update({'out_dim': out_dim})  # add by tianweixi for DeepSTPP mag2 model(for magnitude parameter alpha: )
        ...
        self.q_dec = load_class(self.hparams.q_dec)  # add by tianweixi for DeepSTPP mag2 model
    ...
    def forward(self, st_x, st_y):
        """
        :param st_x: [batch, seq_len, 4] (lat, lon, mag, time)
        :param st_y: [batch, 1, 4]
        """
        batch = st_x.shape[0]
        ...
        t_cum = torch.cumsum(st_x[..., -1], -1) #计算每个seq_len的累计时间，形状依然为[batch_size,seq_len]  addedby tianweixi for magnitude
        ...
        #added by tianweixi for magnitude, here the st_y[...,3]表示选择第四列的数据，因为增加了震级到第3列，所以选择时间应该是最后一列，也就是第4列
        t_ti = tn_ti + st_y[..., -1]  # t - t_i # #计算目标事件相对于所有历史事件的时间间隔 [batch_size, seq_len+num_points]
        

        #added by weixi tian ================
        ##这里st_x[..., 2] 形状 [batch, seq_len]；m_background 形状 [batch, num_points]
        ##torch.cat() 拼接后形状为 [batch, seq_len + num_points]
        m_i = torch.cat((st_x[...,2],m_background),dim = -1) 

        #计算得到，时间的衰减因子b_i,强度缩放系数w_i，以及空间的inv_var（用于计算空间的高斯核密度函数）
        [qm, qv], w_i, b_i, inv_var, q_i = self.encode(st_x)  # add q_i here
            
        # Calculate likelihood 
        sll = torch.log(s_intensity(w_i, b_i, q_i, t_ti, s_diff, inv_var, m_i)) 
        tll = log_ft(t_ti, tn_ti,m_i, w_i, b_i,q_i) #（分别计算了时间和空间的似然函数）
        ...
        return nelbo, sll.mean(), tll.mean(),sll,tll,w_i,b_i,q_i,inv_var,s_diff,m_i,st_x[..., :2]  #added by tianweixi 为了查看数据的变化情况
    ...
    def encode(self, st_x):
        ...
        return [qm, qv], w_i, b_i, inv_var, q_i
    ...
    def calc_lamb(self, st_x, st_x_cum, st_y, st_y_cum, scales, biases,
        ...
        _, w_i, b_i, inv_var, q_i = self.encode(st_x.to(device)) #added by weixi tian
        ...
        for t in tqdm(t_range):
            ...
            if i >= 0:
                ...
                q_i_ = q_i[i:i + 1] #added by weixi tian 
            ...
            #tian weixi add it 本来是：st_x_[..., :-1]，修改为st_x_[..., :2]，目的是为了只获取前2列
