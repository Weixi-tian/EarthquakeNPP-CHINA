...
def scale_ll(dataloader, nll, sll, tll,scales=None):
    ...
    if type(t_scale) == torch.Tensor: #here the type is not the tensor
...
def scale_ll_all_events(dataloader, nll, sll, tll,sll_all,tll_all,scales=None):
    """
    Scale the log likelihoods to the original scale
    ...
    return nll + s_scale + t_scale, sll - s_scale, tll - t_scale, sll_all - s_scale, tll_all - t_scale

def scale_ll_all_events_mag1(dataloader, nll, sll, tll,sll_all,tll_all,scales=None):
    """
    Scale the log likelihoods to the original scale
    ...
    if dataloader is not None:
        assert hasattr(dataloader.dataset, 'max') and hasattr(dataloader.dataset, 'min')
        s0_scale, s1_scale, _, t_scale = dataloader.dataset.max - dataloader.dataset.min
    else:
        assert scales is not None
        s0_scale, s1_scale, t_scale = scales
    if type(t_scale) == torch.Tensor: #here the type is not the tensor
...
def scale_ll_all_events_mag2(dataloader, nll, sll, tll, sll_all,tll_all,scales=None):
    """
    Scale the log likelihoods to the original scale
    ...
    if dataloader is not None:
        assert hasattr(dataloader.dataset, 'max') and hasattr(dataloader.dataset, 'min')
        s0_scale, s1_scale, m_scale,t_scale  = dataloader.dataset.max - dataloader.dataset.min
    else:
        assert scales is not None
        s0_scale, s1_scale, m_scale,t_scale = scales
    ...
    t_scale = np.log(t_scale)
    s_scale = np.log(s0_scale * s1_scale)
    m_scale = np.log(m_scale)


    nll_out = nll + s_scale + t_scale # + m_scale
    sll_out = sll - s_scale #- m_scale
    tll_out = tll - t_scale #- m_scale
    sll_all_out = sll_all - s_scale #- m_scale
    tll_all_out = tll_all - t_scale #- m_scale

    return nll_out, sll_out, tll_out, sll_all_out, tll_all_out
