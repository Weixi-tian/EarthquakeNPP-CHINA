...
class TransformerEncoder(pl.LightningModule):    
    ...
    def __init__(
        ...
        **kwargs
        ...
        if magnitude == False: #新增参数是否使用震级信息
            self.encoder = nn.Linear(3, self.hparams.emb_dim, bias=False) #将数据从3维转换到128维
        else:
            self.encoder = nn.Linear(4, self.hparams.emb_dim, bias=False) #将数据从4维转换到128维
...
#### 定义时间核和空间核--==================================================================================
def temporal_kernel(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type):
    """
    Construct Temporal Kernel λ_i(t)
    :param kernel_type: 'exp' or 'power_law'
        exp:  λ_i(t) = w_i * exp(-b_i * t)
        power law: λ_i(t) = w_i * [exp(-t / τ_i) / (t + c_i)^{1 + ω_i}]
    :return: λ_i(t): [batch, seq_len]
    """
    if temporal_kernel_type == 'power_law':
        lambda_i_t = w_i / torch.pow(t_ti + c_i, 1.0 + omega_i)
    elif temporal_kernel_type == 'exp':
        lambda_i_t = w_i * torch.exp(-b_i * t_ti)
    else:
        raise ValueError(f"Please choose exp or power_law as temporal kernel function")
    return lambda_i_t

def spatial_kernel(s_diff, inv_var, d_i, gamma_i,rho_i, spatial_kernel_type, m_i):
    """
    power_law : f(x, y; m) = (q-1) / (π * D^2 * e^{γ(m - m0)}) * (1 + r^2 / (D^2 * e^{γ(m - m0)}))^{-q}
               = (rho+1 -1)/π/dm_i * (1 + r^2/dm_i)^-(1 + rho_i)
    """
    if spatial_kernel_type == 'gaussian':
        g2 = torch.sum(s_diff * inv_var * s_diff, -1)
        norm_const = torch.sqrt(torch.prod(inv_var, dim=-1)) / (2 * np.pi)  # 归一化常数
        return norm_const * torch.exp(-0.5 * g2)
    elif spatial_kernel_type == 'power_law':
        spatial_distance_squared = torch.sum(s_diff** 2, dim=-1)
        dm_i = d_i * torch.exp(gamma_i * (m_i))
        norm_const = rho_i / (np.pi * dm_i)
        return norm_const * torch.pow( 1 + spatial_distance_squared / dm_i, -(1.0 + rho_i))
    else:
        raise ValueError(f"Unknown spatial_kernel_type: {spatial_kernel_type}")

def triggering_kernel(q_i, m_i):
    return torch.exp(q_i * m_i)

def s_intensity(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type,
                s_diff, inv_var, d_i, gamma_i,rho_i, spatial_kernel_type, m_i):
    lambda_i_t = temporal_kernel(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type)
    weights = lambda_i_t / torch.sum(lambda_i_t, -1).unsqueeze(-1)  # soft weights
    f_s_cond_t = torch.sum(weights * spatial_kernel(s_diff, inv_var, d_i, gamma_i,rho_i, spatial_kernel_type, m_i), -1)
    return f_s_cond_t

def t_intensity(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type,
                q_i, m_i, magnitude_information,
                mu, background_rate, area=1):
    """
    λ(t) = Σ_i λ_i(t)
    ...
    if background_rate == True: 
        return torch.sum(lambda_i_t, -1) + mu * area
    else:
        return torch.sum(lambda_i_t, -1)

def temporal_integral(w_i, b_i, c_i, omega_i, tn_ti, t_ti, temporal_kernel_type,
                      q_i, m_i, magnitude_information, 
                      mu, background_rate, area=1):
    """
    -∫_{t_n}^t λ(t') dt' 
    ...
    Returns:
        integral: [batch]
    """
    if magnitude_information == True: # using magnitude kernel 
        if temporal_kernel_type == 'exp':
            if background_rate == True:
                return -(mu * area * (t_ti[:,-1] - tn_ti[:,-1])) + torch.sum((w_i / b_i * triggering_kernel(q_i, m_i)) * (torch.exp(-b_i * t_ti) - torch.exp(-b_i * tn_ti)), -1)
            else:
                return torch.sum((w_i / b_i * triggering_kernel(q_i, m_i))* (torch.exp(-b_i * t_ti) - torch.exp(-b_i * tn_ti)), dim=-1)
            
        elif temporal_kernel_type == 'power_law':
            if background_rate == True:
                integral = (torch.pow(t_ti + c_i, -omega_i) - torch.pow(tn_ti + c_i, -omega_i))/ (omega_i)
                return -(mu * area * (t_ti[:, -1] - tn_ti[:, -1])) + torch.sum(( w_i * triggering_kernel(q_i, m_i)) * integral, dim=-1) 
            else:
                integral = (torch.pow(t_ti + c_i, -omega_i) - torch.pow(tn_ti + c_i, -omega_i))/ (omega_i)
                return torch.sum((w_i * triggering_kernel(q_i, m_i)) * integral, dim=-1) 
        else:
            raise ValueError(f"Unsupported temporal_kernel_type: {temporal_kernel_type}")
        
    else:  # don't using magnitude kernel 
        if temporal_kernel_type == 'exp':
            if background_rate == True:
                return -(mu * area * (t_ti[:,-1] - tn_ti[:,-1])) + torch.sum((w_i / b_i ) * (torch.exp(-b_i * t_ti) - torch.exp(-b_i * tn_ti)), -1)
            else:
                return torch.sum((w_i / b_i )* (torch.exp(-b_i * t_ti) - torch.exp(-b_i * tn_ti)), dim=-1)
            
        elif temporal_kernel_type == 'power_law':
            if background_rate == True:
                integral = (torch.pow(t_ti + c_i, -omega_i) - torch.pow(tn_ti + c_i, -omega_i))/ (omega_i)
                return -(mu * area * (t_ti[:, -1] - tn_ti[:, -1])) + torch.sum(w_i * integral, dim=-1) 
            else:
                integral = (torch.pow(t_ti + c_i, -omega_i) - torch.pow(tn_ti + c_i, -omega_i))/ (omega_i)
                return torch.sum(w_i * integral, dim=-1) 
        else:
            raise ValueError(f"Unsupported temporal_kernel_type: {temporal_kernel_type}")


def intensity(w_i, b_i, t_ti, c_i, omega_i,temporal_kernel_type, 
              s_diff, inv_var, d_i, gamma_i,rho_i, spatial_kernel_type, 
              q_i, m_i, magnitude_information,
              mu, background_rate, area=1 ):
    
    if magnitude_information == True:
        if background_rate == True:
            return mu + torch.sum(triggering_kernel(q_i, m_i) * temporal_kernel(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type)*
                                spatial_kernel(s_diff, inv_var, d_i, gamma_i,rho_i, spatial_kernel_type, m_i), -1)
        elif background_rate == False:
            return  triggering_kernel(q_i, m_i) \
                    * t_intensity(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type, mu, background_rate,area=1)  \
                    * s_intensity(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type,
                                    s_diff, inv_var, d_i, gamma_i,rho_i, spatial_kernel_type, m_i)
        else:
            raise ValueError(f"Intensity function constructed Wrong")
    else:
        if background_rate == True:
            return mu + torch.sum(temporal_kernel(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type) *
                                spatial_kernel(s_diff, inv_var, d_i, gamma_i,rho_i, spatial_kernel_type, m_i), -1)
        elif background_rate == False:
            return t_intensity(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type, mu, background_rate,area=1)  \
                    * s_intensity(w_i, b_i, c_i, omega_i, t_ti, temporal_kernel_type,
                                    s_diff, inv_var, d_i, gamma_i,rho_i, spatial_kernel_type, m_i)
        else:
            raise ValueError(f"Intensity function constructed Wrong")
...
class DeepSTPointProcess(BaseSTPointProcess):
    ...
    def __init__(
        ...
        magnitude_only_for_latent_Variable : bool = False,
        ...
        **kwargs  # for BaseSTPointProcess
        ...
        q_dec['init_args'].update({'out_dim': out_dim})
        c_dec['init_args'].update({'out_dim': out_dim})
        omega_dec['init_args'].update({'out_dim': out_dim})
        d_dec['init_args'].update({'out_dim': out_dim})
        gamma_dec['init_args'].update({'out_dim': out_dim})
        rho_dec['init_args'].update({'out_dim': out_dim})
        ...
        self.q_dec = load_class(self.hparams.q_dec)
        self.c_dec = load_class(self.hparams.c_dec)
        self.omega_dec = load_class(self.hparams.omega_dec)
        self.d_dec = load_class(self.hparams.d_dec)
        self.gamma_dec = load_class(self.hparams.gamma_dec)
        self.rho_dec = load_class(self.hparams.rho_dec)
        ...
        self.background_mag = nn.Parameter(torch.zeros((self.num_points, 1)), requires_grad=True) #(num_points,1)随机背景点的初始震级

        self.background_rate = background_rate  # 保存成类变量
        ...
        self.temporal_kernel_type = temporal_kernel_type
        self.spatial_kernel_type = spatial_kernel_type
        self.magnitude_information = magnitude_information
        self.magnitude_only_for_latent_Variable = magnitude_only_for_latent_Variable
    ...
    def forward(self, st_x, st_y, boundary_area):
        ...
        m_background = self.background_mag.unsqueeze(0).repeat(batch, 1, 1).squeeze(-1)  # # (batch, num_points, 1) ——(batch, num_points)
        ...
        t_cum = torch.cumsum(st_x[..., -1], -1) #计算每个seq_len的累计时间，形状依然为[batch_size,seq_len]
        ...
        t_ti = tn_ti + st_y[..., -1]  # t - t_i # #计算目标事件相对于所有历史事件的时间间隔 [batch_size, seq_len+num_points]

        if self.magnitude_information == True:
            if self.magnitude_only_for_latent_Variable == False:
                m_i = torch.cat((st_x[...,-2],m_background),dim = -1) ## torch.cat() 拼接后形状为 [batch, seq_len + num_points]
            elif self.magnitude_only_for_latent_Variable == True:
        ...
        if self.background_rate == True:
            ...
            sll = torch.log(s_intensity(w_i, b_i, c_i, omega_i, t_ti, self.temporal_kernel_type,
        ...
        if self.magnitude_information == False:
            return nelbo, sll.mean(), tll.mean(), sll, tll, w_i, b_i, inv_var, s_diff, st_x[..., :2] 
        else:
            return nelbo, sll.mean(), tll.mean(), sll, tll, w_i, b_i,  inv_var, s_diff, q_i, m_i, st_x[..., :2]
    ...
    def encode(self, st_x):
        ...
        q_i = self.q_dec.decode(z)
        c_i = self.c_dec.decode(z)
        omega_i = self.omega_dec.decode(z)
        gamma_i = self.gamma_dec.decode(z)
        d_i = self.d_dec.decode(z)
        rho_i = self.rho_dec.decode(z)
        if self.spatial_kernel_type == 'power_law':
            rho_i = torch.clamp(rho_i, max=5)  #规定rho_i的值，避免爆炸
        
        return [qm, qv], w_i, b_i, inv_var, q_i, c_i, omega_i, d_i, gamma_i, rho_i
    ...
    def calc_lamb(self, st_x, st_x_cum, st_y, st_y_cum, scales, biases,
        ...
        for t in tqdm(t_range):
            ...
            s_x_ = torch.cat((st_x_[..., :2], background), 1).repeat(N, 1, 1).to(device)
